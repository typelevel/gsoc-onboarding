...
val richochetclementine1315: Contributor = Contributor("richochetclementine1315"):
  ...
  SignallingRef[IO].of(0).toResource.flatMap { count =>
    SignallingRef[IO].of("🥚").toResource.flatMap { emoji =>
      ...
      autoClicker *> div(
        ...
        p(styleAttr := "color:#6c7086; font-size:12px; margin:0 0 16px;",
        ...
        div(
          ...
          """,
          count.map(n => evolve(n)),
          onClick --> (_.foreach(_ => count.update(_ + 1)))
        ),

        // Level name
        p(styleAttr := "font-size: 18px; font-weight: bold; margin: 4px 0;",
          count.map(n => levelName(n))),

        // Click count
        p(styleAttr := "font-size: 14px; color: #89b4fa; margin: 4px 0;",
          count.map(n => s"Clicks: $n")),

        // Progress hint
        p(styleAttr := "font-size: 12px; color:#6c7086; margin: 4px 0;",
          count.map(n => nextAt(n))),

        // Auto clicker note
        p(styleAttr := "font-size: 11px; color:#45475a; margin-top: 16px;",
